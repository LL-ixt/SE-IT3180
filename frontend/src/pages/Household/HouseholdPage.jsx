...
import { Plus, Edit, Trash2, Search, Users, Home as HomeIcon, X } from 'lucide-react';
...
const HouseholdPage = () => {
    ...
    return (
        ...
        <ContentWrapper title="Quản lý Hộ Khẩu">
            ...
            {/* Statistics Cards - Đã sửa style sang dạng kính (Glass) */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                {[
                    { label: 'Tổng số căn hộ', value: totalApartments, icon: HomeIcon, color: 'text-blue-400', bg: 'bg-blue-400/10' },
                    { label: 'Đang có người ở', value: occupiedApartments, icon: HomeIcon, color: 'text-green-400', bg: 'bg-green-400/10' },
                    { label: 'Tổng nhân khẩu', value: totalMembers, icon: Users, color: 'text-purple-400', bg: 'bg-purple-400/10' },
                    { label: 'Tổng diện tích', value: `${totalArea}m²`, icon: HomeIcon, color: 'text-orange-400', bg: 'bg-orange-400/10' },
                ].map((stat, index) => (
                    <div key={index} className="bg-white/5 border border-white/10 p-4 rounded-xl shadow-lg backdrop-blur-sm">
